...
<div class="main-content">
    ...
    <div class="page-content">
        <div class="container-fluid">
            @php $button = auth()->user()->hasPermissions('civadd')?'Add CIV':''; @endphp
            <x-bar-after-top-bar title="{{ $title }}" button="" count="{{ count($civs) }}" target="addciv" currentmonth=""/>
            <!-- start page title -->
            {{-- <div class="row">
                <div class="col-6">
                            ...
                            <div class="table-responsive table-card">
                                <table  class="table table-bordered table-hover table-nowrap align-middle mb-0" id="mytab">
                                    <tr>
                                        <thead>
                                            <tr>
                                                @if(auth()->user()->hasPermissions('civdelete') || auth()->user()->hasPermissions('civedit'))
                                    ...
                                    <tbody>
                                        @forelse ($civs as $civ)
                                        @php $civdata = \App\Models\Items::where('parent_item_id',$civ->id)->where('property_type','civs')->get();
                                        $rowspan = count($civdata);
                                        @endphp
                                        <tr>
                                            @if(auth()->user()->hasPermissions('civdelete') || auth()->user()->hasPermissions('civedit'))
                                            <td rowspan="{{$rowspan+1}}" class="text-center fixed">
                                                @if(auth()->user()->hasPermissions('civdelete'))
                                                ...
                                                @endif
                                                @if(auth()->user()->hasPermissions('civedit'))
                                                    <a href="{{ route('civedit',[$civ->id]) }}" class="btn btn-primary btn-sm" id="{{ $civ->id }}"><i class="las la-pen-alt fs-18 align-middle"></i></a>
                                                @endif
                                            </td>
                                            @endif
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->issue_voc_no }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->issue_date }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->issue_unit }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->issue_station }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->receipt_voc_no }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->receipt_date }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->receipt_unit }}</td>
                                            <td rowspan="{{$rowspan+1}}" class="text-wrap">{{ $civ->receipt_station }}</td>
                                            @foreach($civdata as $item)
                                                <tr class="table-secondary">
                                                    <td>{{ $civ->issue_voc_no }}</td>
                                                    <td>{{ $item['lpno'] }}</td>
                                                    <td>{{ $item['items'] }}</td>
                                                    <td>{{ $item['au'] }}</td>
                                                    <td>{{ $item['date'] }}</td>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <table class="table table-bordered table-hover table-nowrap align-middle mb-0" id="myTable">
                            ...
                            <tbody>
                                <tr>
                                    <td><input type="text" name="lpno[]" class="form-control" placeholder="Item Name"></td>
                                    <td><input type="text" name="items[]" class="form-control" placeholder="A/U"></td>
    ...
    $('#civ-form').on('submit', function(event) {
        ...
        var tableData = [];
                $('#myTable tbody tr').each(function() {
                    ...
                    tableData.push({ lpno:lpno,items:items,date:date,qty:qty,remarks:remarks });
                });
