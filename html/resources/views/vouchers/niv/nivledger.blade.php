...
<style>    
    ...
    <div class="page-content">
        <div class="container-fluid">
            {{-- NIV LEDGER FOR THE MONTH OF {{ now()->format('M, Y') }} --}}
            <x-bar-after-top-bar title="{{ $title }}" button="" count="0" target="addcrv" currentmonth="SHOWING NIV LEDGER OF CURRENT MONTH ({{ now()->format('F') }})"/>
            <!-- start page title -->
            {{-- <div class="row">
                <div class="col-6">
            ...
            <div class="row">
                <div class="col-xs-6">
                        ...
                        @endphp
                    <form action="{{ route('nivledger',[request()->route()->parameters['vocid']]) }}">
                        <div class="date-range-picker float-start">
                            ...
                            <a href="{{ route('nivledger',[request()->route()->parameters['vocid']]) }}" class="btn btn-primary float-end btn-sm mt-4">
                                <i class="las la-redo-alt"></i>
                            </a>                                                    
                        </div>                                                     
                        </form>                        
                        <form action="{{ route('nivledgerdatadownload',[request()->route()->parameters['vocid']]) }}" method="POST" class="float-end">
                                @csrf
                                ...
                                <table  class="table table-bordered table-hover table-nowrap align-middle mb-0" id="grid-selection">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-column-id="date">Nomenclature</th>
                                            <th data-column-id="items">A/U</th>
                                    ...
                                    <tbody>
                                        ...
                                        @forelse ($nivledger as $niv)
                                            <tr>
                                                ...
                                                <td>{{ $niv['items'] }}</td>
                                                <td>{{ $niv['au'] }}</td>
                                                <td class="text-center">{{ $niv['date'] }}</td>
                                                <td class="text-center">{{ $niv['qty'] }}</td>
                                                <td class="text-center">{{ $niv['remarks'] }}</td>
                                            </tr>      
                                        @empty
